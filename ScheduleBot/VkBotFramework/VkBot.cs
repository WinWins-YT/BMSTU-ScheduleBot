	...
	public partial class VkBot : IVkBot
	{
		...
		private void RegisterDefaultDependencies(IServiceCollection container)
		{
			...
			if (container.All(x => x.ServiceType != typeof(IVkApi)))
			{
				...
				services1.AddSingleton(new HttpClient() { Timeout = TimeSpan.FromSeconds(600) });
				var vkApiByDefault = new VkApi(services1);
		...
		private void ProcessLongPollEvents(BotsLongPollHistoryResponse pollResponse)
		{
			foreach (GroupUpdate update in pollResponse.Updates)
			{
				...
				if (update.Instance is MessageNew)
				{
					long peerId = (update.Instance as MessageNew).Message.PeerId.Value;
					...
					OnMessageReceived?.Invoke(this, new MessageReceivedEventArgs((update.Instance as MessageNew).Message, peerContext));
					this.SearchTemplatesMatchingMessageAndHandle((update.Instance as MessageNew).Message, peerContext);
				}
			}
		}
